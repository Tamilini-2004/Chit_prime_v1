...
class FirebaseService {
  ...
  static Future<void> createOrUpdateUser({
    ...
    final isAdminRole = role == FirebaseKeys.roleAdmin ||
        role == FirebaseKeys.roleSuperAdmin;
    if (!doc.exists) {
      await ref.set({
        'uid': uid,
        'phone': phone,
        ...
        'kycStatus':
            isAdminRole ? FirebaseKeys.statusVerified : 'draft',
        ...
        'state': '',
        'fcmToken': '',
        'registrationCompleted': isAdminRole,
        'isOnline': true,
  ...
  static Future<void> submitMemberKyc({
    required String uid,
    required String name,
    required String email,
    required String aadhaarNumber,
    required Uint8List aadhaarImageBytes,
    required String aadhaarFileName,
  }) async {
    final lowerName = aadhaarFileName.toLowerCase();
    final mimeType = lowerName.endsWith('.png') ? 'image/png' : 'image/jpeg';
    final inlineImage =
        'data:$mimeType;base64,${base64Encode(aadhaarImageBytes)}';

    await _db.collection(FirebaseKeys.users).doc(uid).set({
      'uid': uid,
      'name': name,
      'email': email,
      ...
      'aadhaarImageUrl': '',
      'aadhaarImageData': inlineImage,
      'kycStatus': FirebaseKeys.statusPending,
  ...
  static Future<void> updateCreditScore(String uid, int delta) async {
    ...
    await _db
        .collection(FirebaseKeys.users)
        .doc(uid)
        .collection(FirebaseKeys.scoreHistory)
        .add({
      'change': delta,
  ...
  static Future<void> verifyKyc(String uid) async {
    ...
    await sendNotification(
      ...
      title: '✅ KYC Verified!',
      message:
          'Your KYC documents have been verified. You now have full access.',
      type: 'system',
      actionRoute: '/profile',
    );
  }
  ...
  static Future<Map<String, String>> createGroup({
    required String name,
    ...
    final groupCode = AppUtils.generateGroupCode();
    ...
    return {'groupId': ref.id, 'groupCode': groupCode};
  }
  ...
  static Future<void> joinGroup(String groupId) async {
    ...
    final creditScore =
        (userDoc.data()?['creditScore'] as num?)?.toInt() ?? 650;
    ...
    // Notify foreman
    final groupDoc =
        await _db.collection(FirebaseKeys.groups).doc(groupId).get();
  ...
  static Future<GroupModel?> getGroupByCode(String code) async {
    ...
    return GroupModel.fromDoc(query.docs.first);
  }

  static Future<String> joinGroupByCode(String code) async {
    final group = await getGroupByCode(code);
    if (group == null) {
      throw Exception('Group not found for code $code');
    }
    if (group.memberUids.contains(currentUid)) {
  ...
  static Future<String> joinByCode(String code) async {
    return joinGroupByCode(code);
  }

  static Future<String> moneyRequest({
    ...
    final ref = _db.collection(FirebaseKeys.moneyRequests).doc();
    final now = DateTime.now();
    await ref.set({
      'requestId': ref.id,
      'groupId': groupId,
  ...
  static Future<String> pollFromRequest({
    ...
    final requestDoc =
        await _db.collection(FirebaseKeys.moneyRequests).doc(requestId).get();
    if (!requestDoc.exists) {
      throw Exception('Money request not found');
    }

    final ref = _db.collection(FirebaseKeys.polls).doc();
    await ref.set({
      'pollId': ref.id,
      'requestId': requestId,
      'groupId': groupId,
    ...
    await _db.collection(FirebaseKeys.moneyRequests).doc(requestId).update({
      'pollId': ref.id,
      'status': 'poll_created',
      'updatedAt': FieldValue.serverTimestamp()
    });
  ...
  static Future<String> fundFromPoll({
    ...
    final pollDoc = await _db.collection(FirebaseKeys.polls).doc(pollId).get();
    if (!pollDoc.exists) {
      throw Exception('Poll not found');
    }

    await _db.collection(FirebaseKeys.polls).doc(pollId).update({
      'status': 'approved',
      'updatedAt': FieldValue.serverTimestamp(),
    });

    final ref = _db.collection(FirebaseKeys.funds).doc();
    await ref.set({
      'fundId': ref.id,
      'pollId': pollId,
      'groupId': groupId,
  ...
  static Future<Map<String, dynamic>> repayment({
    required String groupId,
    required String groupName,
    required double amount,
    ...
    batch.set(_db.collection(FirebaseKeys.repayments).doc(txnId), {
      'repaymentId': txnId,
      'groupId': groupId,
      'groupName': groupName,
      'userId': uid,
    ...
    if (foremanUid.isNotEmpty && foremanUid != uid) {
      await sendNotification(
        ...
        message:
            '$name repaid ${AppUtils.formatCurrency(amount)} to $groupName',
        type: 'payment',
        relatedId: txnId,
        actionRoute: '/groups/$groupId',
      );
  ...
  static Future<void> updateGroupStatus(String groupId, String status) async {
    ...
    await _logAdminAction(groupId,
        status == 'terminated' ? 'suspend_group' : 'activate_group', 'group');
  }
  ...
  static Stream<List<ContributionModel>> userContributionsStream(String uid) {
    return _db
        .collection(FirebaseKeys.contributions)
  ...
  static Stream<List<ContributionModel>> groupContributionsStream(
      String groupId) {
    return _db
        .collection(FirebaseKeys.contributions)
  ...
  static Stream<List<AuctionModel>> groupAuctionsStream(String groupId) {
    return _db
        .collection(FirebaseKeys.auctions)
        .where('groupId', isEqualTo: groupId)
        .orderBy('createdAt', descending: true)
        .snapshots()
  ...
  static Future<String> openAuction(String groupId) async {
    ...
    await ref.set({
      ...
      'auctionType': group.fundType,
      'startTime': Timestamp.fromDate(now),
      'endTime': Timestamp.fromDate(
          now.add(Duration(hours: group.auctionWindowHours))),
    ...
    for (final m in members.docs) {
      ...
      batch.set(nRef, {
        ...
        'message':
            'Cycle ${group.currentCycle} auction for ${group.groupName} is now live!',
        'isRead': false,
        'relatedId': ref.id,
        'relatedType': 'auction',
  ...
  static Stream<List<PayoutModel>> groupPayoutsStream(String groupId) {
    return _db
        .collection(FirebaseKeys.payouts)
        .where('groupId', isEqualTo: groupId)
        .orderBy('createdAt', descending: true)
        .snapshots()
  ...
  static Future<void> confirmPayout(String payoutId) async {
    ...
    if (winnerUid.isNotEmpty) {
      await sendNotification(
        ...
        message:
            '${AppUtils.formatCurrency(netPayout)} transferred to your bank from $groupName!',
        type: 'payment',
        relatedId: payoutId,
  ...
  static Stream<List<FraudAlertModel>> fraudAlertsStream() {
    return _db
        .collection(FirebaseKeys.fraudAlerts)
        .orderBy('detectedAt', descending: true)
        .snapshots()
        .map((s) => s.docs.map(FraudAlertModel.fromDoc).toList());
  }

  static Future<void> updateFraudAlert(String alertId, String status,
      {String notes = ''}) async {
  ...
  static Stream<Map<String, dynamic>> settingsStream() {
    return _db
        .collection(FirebaseKeys.settings)
        .doc('platform')
        .snapshots()
  ...
  static Future<void> _logAdminAction(
      String targetId, String actionType, String targetType) async {
    final uid = currentUid;
    if (uid.isEmpty) return;
    try {
      final userDoc = await _db.collection(FirebaseKeys.users).doc(uid).get();

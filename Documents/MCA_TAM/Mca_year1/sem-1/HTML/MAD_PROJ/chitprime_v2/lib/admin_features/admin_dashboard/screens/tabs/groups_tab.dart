...
class _GroupsTabState extends ConsumerState<GroupsTab> {
                    ...
                    child: Container(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          ...
                          color:
                              _filter == f ? Colors.white : AppColors.primary,
                        ),
                      ),
                    ),
                  ))
                  ...
                  return AppCard(
                    ...
                    child: Column(
                        ...
                        children: [
                          Row(children: [
                            ...
                            Expanded(
                                child: Column(
                                  ...
                                  Text(
                                      '${g.currentMembers}/${g.totalMembers} • ${g.fundType.toUpperCase()} • by ${g.foremanName.isNotEmpty ? g.foremanName : 'Admin'}',
                                      style: const TextStyle(
                                          fontSize: 11,
                          ...
                          Row(children: [
                            _chip('Cycle ${g.currentCycle}/${g.cycleDuration}',
                                AppColors.primary),
                            const SizedBox(width: 8),
                            _chip(
                                '${AppUtils.formatCurrency(g.monthlyContribution)}/mo',
                                AppColors.secondary),
                            const SizedBox(width: 8),
                            _chip('₹${AppUtils.formatCurrency(g.totalFund)}',
                                AppColors.accent),
                          ]),
                          ...
                          Row(children: [
                            ...
                            Expanded(
                                child: _btn(
                                    context, 'Auction', AppColors.secondary,
                                    () async {
                              await ref
                                  .read(auctionNotifierProvider.notifier)
                                  .openAuction(g.groupId);
                              if (context.mounted)
                                showSnack(context,
                                    'Auction opened! Members notified 🔨');
                            })),
                            const SizedBox(width: 8),
                            Expanded(
                                child: _btn(
                              ...
                              () async {
                                if (g.isActive) {
                                  await ref
                                      ...
                                      .suspendGroup(g.groupId);
                                } else {
                                  await ref
                                      .read(groupsNotifierProvider.notifier)
                                      .reactivateGroup(g.groupId);
                                }
                                if (context.mounted)
                                  showSnack(context,
                                      'Group ${g.isActive ? 'suspended' : 'activated'}');
                              },
                            )),
                          ]),
                        ]),
        ...
        builder: (ctx, setS) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              _dialogSlider('Total Members', '$members', members.toDouble(), 2,
                  50, (v) => setS(() => members = v.toInt())),
              _dialogSlider(
                  'Duration (months)',
                  '$duration months',
                  duration.toDouble(),
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        await _notifyAllUsers(
                          groupId: groupId,
                          groupName: nameCtrl.text.trim().isEmpty
                              ? 'Admin Chit Group'
                              : nameCtrl.text.trim(),
                          contribution: contribution,
                        );
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                _detailRow('Contribution',
                                                    '₹${contribution.toInt()} / month'),
                                                _detailRow('Members',
                                                    '$members total'),
                                                _detailRow('Duration',
                                                    '$duration months'),
                                                _detailRow('Fund Type',
                                                    fundType.toUpperCase()),
                                                _detailRow('Privacy', 'Public'),
                                              ],
                                            ),
                                          ),
                                        ]),
                        ...
                        if (ctx.mounted)
                          showSnack(context, 'Failed: $e', isError: true);
                      } finally {
                        setS(() => loading = false);
                      }
                    },
  ...
  Future<void> _notifyAllUsers({
    ...
    for (final u in users.docs) {
      ...
      batch.set(notifRef, {
        ...
        'message':
            'Admin created "$groupName" — ${AppUtils.formatCurrency(contribution)}/month. Join now!',
        'isRead': false,
        'relatedId': groupId,
        'relatedType': 'group',
        'actionRoute': '/groups',
  ...
  Widget _dialogSlider(String label, String value, double current, double min,
